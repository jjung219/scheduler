...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the save button shows, Change the name
    expect(getByText(appointment, "Save")).toBeInTheDocument();
    fireEvent.change(getByPlaceholderText(appointment, /Enter student name/i), { target: { value: "Jenny Jung"}});
    // 5. Click the "Save" button
    fireEvent.click(getByText(appointment, "Save"));
    // 6. Check that the "Saving" text is displayed
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
    // 7. Wait until the element with the "Delete" button is displayed
    await waitForElement(() => getByAltText(appointment, "Delete"));
    // 8. Check that the DayListItem with the text "Monday" still has the text "2 spots remaining"
    const day = getAllByTestId(container, "day").find(day => queryByText(day, "Monday"))
    expect(getByText(day, "2 spots remaining")).toBeInTheDocument();
